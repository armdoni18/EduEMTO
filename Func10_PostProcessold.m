function [] = Func10_PostProcessold(fem,~,~)
...
% Az=zeros(NodeY,NodeX);                  % rearrange the Az distribution in the model format
% 
% Xaxis=zeros(NodeX,1);
% Yaxis=zeros(NodeY,1);
% 
...
% Plot Coil2
    ...
    % figure(8);
    % 
    % fill(IronX1,IronY1,"w"); hold on;
    % fill(IronX2,IronY2,"w"); hold on;
    % fill(Coil1X1,Coil1Y1,"w");hold on;
    % fill(Coil2X1,Coil2Y1,"w");hold on;
    ...
    % xlabel('X Coordinate');
    % ylabel('Y Coordinate');
    % title('Design Setting and Vector Potential Contour Lines');
    % grid on;
